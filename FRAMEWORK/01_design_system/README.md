...
4. **الإنشاء عند الطلب في المشاريع (On-Demand / JIT Generation)**:
   - عند بناء صفحات في المشاريع المستهدفة، يُنشئ الـ AI المكونات المطلوبة فقط لتلك الصفحة (Blade, React, Vue) ولا يقوم بترحيل المكونات الـ 23 دفعة واحدة.
5. **التحويل للتقنية الأصلية (Native Component Adaptation)**:
   - يتم تحويل كود المكون من HTML/CSS/JS الخالص إلى قوالب التقنية المستخدمة (مثل `<x-ui.button>` في Laravel Blade أو `<Button />` في React) مع الحفاظ التام على نفس فئات الـ CSS والـ Tokens.
