...
### 1. مراجعة اتساق الـ Stack ونمط الاتصال (`stack.yaml`)
...
- **فحص نمط الاتصال المعماري:** تحقق من وجود `architecture.communication.mode` وأنه يحمل إحدى القيم المعتمدة (`direct` / `api_first` / `hybrid`)، وتطابق قنوات الاستهلاك (`web` / `mobile`).
