...
// 1. Extract exact section modals (lines 4690 to 4910)
const startTagRegex = /<section\s+id="modals"[\s\S]*?>/i;
...
// 2. Extract All 8 Modal Dialogs (lines 7080 to 8015)
...
<head>
    ...
    <title>13. النوافذ المنبثقة والحوارية (Modals & Dialogs) — Taqniya Design System</title>
    <!-- Fonts -->
...
<body class="min-h-screen bg-[var(--bx-canvas)] text-bx-text font-sans antialiased">
    ...
    <div class="max-w-[1600px] mx-auto p-4 sm:p-8 lg:p-12 space-y-8">
        ...
        <header class="flex items-center justify-between border-b border-bx-border pb-4">
            <div class="flex items-center gap-3">
                ...
                <span class="text-xs font-black text-bx-title">13. النوافذ المنبثقة والحوارية (Modals & Dialogs)</span>
            </div>
    ...
    <!-- All 8 Spatial Modal Dialog Overlays -->
    ${allModalsText}
...
// Write files
const modalFolder = path.join(targetDir, 'components/13-modal');
fs.writeFileSync(path.join(modalFolder, 'index.html'), fullHTML, 'utf8');
fs.writeFileSync(path.join(modalFolder, 'component.js'), componentJS, 'utf8');

console.log('13-modal rebuilt successfully with all 8 modals and full wizard engine!');
